...
function DoSigning({ session, userName }) {
  ...
  function shareDoWhatsApp(d) {
    const lines = [
      '📋 *MJM Nursery — Delivery Order*',
      `DO No: ${d.do_number || '—'}`,
      `Date: ${d.delivery_date || '—'}`,
      `Customer: ${activeAL?.customer_name || d.remark || '—'}`,
      `Order: ${activeAL?.order_number || '—'}`,
      `Total Qty: ${d.total_qty || 0} seedlings`,
    ];
    if (d.image_url) {
      lines.push('', '📄 *View / Download DO Document:*');
      lines.push(d.image_url);
    }
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {doRows === null ? (
                      ...
                      doRows.map((d) => <DoRow key={d.id} d={d} plotMap={plotMap} onPrint={() => doPrint(d)} onShare={() => shareDoWhatsApp(d)} />)
                    )}
                  </tbody>
                </table>
              </div>
...
// ── Small helpers ──
function DoRow({ d, plotMap, onPrint, onShare }) {
    ...
    <tr className={isCancelled ? 'opacity-50' : ''}>
      ...
      <td>
        <div className="flex items-center gap-1.5">
          ...
          {d.image_url && (
            <button onClick={onShare} title="Share DO document via WhatsApp"
              className="w-11 h-11 rounded-lg border border-slate-200 hover:border-green-400 hover:bg-green-50 flex items-center justify-center cursor-pointer transition-colors bg-slate-50 shrink-0">
              <WhatsAppIcon />
            </button>
          )}
